  ...
  const fetchSiteSettings = async () => {
    try {
      ...
      const settings: SiteSettings = {
        site_name: settingsData.find(s => s.id === 'site_name')?.value || 'LUXXBIO LABS',
        site_logo: settingsData.find(s => s.id === 'site_logo')?.value || '/luxxbio-logo.png',
        ...
        hero_badge_text: settingsData.find(s => s.id === 'hero_badge_text')?.value || 'Precision. Purity. Performance.',
        ...
        hero_description: settingsData.find(s => s.id === 'hero_description')?.value || 'LUXXBIO LABS provides research-grade peptides engineered for precision, purity, and consistency.',
